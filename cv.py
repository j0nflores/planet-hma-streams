...
with tf.device('/CPU:0'):
    X_train, X_test, y_train, y_test = prep_data(imgs_path,masks_path,0.5,rand,k_class)
...
model.fit(train_data, verbose=0, epochs=2, 
...
with tf.device('/CPU:0'):
    ...
    y_test,y_pred = postprocess_cv(y_test,y_pred,k_class,m2b)
